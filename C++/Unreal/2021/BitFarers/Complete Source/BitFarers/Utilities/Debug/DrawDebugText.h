...
BITFARERS_API void DrawTextDebugLine( const UWorld* InWorld, FVector const& LineStart, FVector const& LineEnd, FColor const& Color, bool bPersistentLines = false, float LifeTime = -1.f, uint8 DepthPriority = 0, float Thickness = 0.f );

BITFARERS_API void DrawDebugCharacter( const UWorld* InWorld, char Character, FVector const& Location, FColor const& Color, bool bPersistentLines = false, float LifeTime = -1.f, uint8 DepthPriority = 0, float Thickness = 0.f, float Size = 1, FRotator Rotation = FRotator::ZeroRotator );

BITFARERS_API void DrawDebugSegmentById( const UWorld * InWorld, uint32 SegmentId, FVector const & Location, FColor const & Color, bool bPersistentLines, float LifeTime, uint8 DepthPriority, float Thickness, float Size = 1, FRotator Rotation = FRotator::ZeroRotator );

BITFARERS_API void DrawDebugText( const UWorld* InWorld, FString Text, FVector const& Location, FColor const& Color, bool bPersistentLines = false, float LifeTime = -1.f, uint8 DepthPriority = 0, float Thickness = 0.f, float Size = 1, bool bRotateToCamera = true, bool bHasMaxDistance = true, int32 MaxDistance = 2500, bool bCapitalize = false );
